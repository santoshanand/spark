...
type (
	// Router - router struct to hold all the route
	Router struct {
		trees  [21]*node
		routes []Route
		spark  *Spark
...
// NewRouter - func to create new router
func NewRouter(e *Spark) (r *Router) {
	r = &Router{
...
// Add - To add a new method with path, handler func and application
func (r *Router) Add(method, path string, h HandlerFunc, e *Spark) {
...
func (r *Router) insert(method, path string, h HandlerFunc, t ntype, pnames []string, e *Spark) {
	// Adjust max param
	l := len(pnames)
	if *e.maxParam < l {
		*e.maxParam = l
	...
	for {
		...
		if l == 0 {
			...
			// Split node
			n := newNode(cn.typ, cn.prefix[l:], cn, cn.children, cn.handler, cn.pnames, cn.spark)
...
func newNode(t ntype, pre string, p *node, c children, h HandlerFunc, pnames []string, e *Spark) *node {
	return &node{
		typ:      t,
		label:    pre[0],
		prefix:   pre,
...
// Find - to find a method with pach and context
func (r *Router) Find(method, path string, ctx *Context) (h HandlerFunc, e *Spark) {
	cn := r.trees[r.treeIndex(method)] // Current node as root
	search := path
...
func (r *Router) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	c := r.spark.pool.Get().(*Context)
	h, _ := r.Find(req.Method, req.URL.Path, c)
	c.reset(req, w, r.spark)
	if h == nil {
