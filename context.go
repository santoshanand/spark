...
func NewContext(req *http.Request, res *Response, e *Spark) *Context {
	return &Context{
		request:  req,
		response: res,
		spark:    e,
...
func (c *Context) Bind(i interface{}) error {
	return c.spark.binder(c.request, i)
}

// Render renders a template with data and sends a text/html response with status
// code. Templates can be registered using `saprk.SetRenderer()`.
func (c *Context) Render(code int, name string, data interface{}) (err error) {
	if c.spark.renderer == nil {
		return ErrRendererNotRegistered
	}
	c.response.Header().Set(ContentType, TextHTML)
	c.response.WriteHeader(code)
	if err = c.spark.renderer.Render(c.response, name, data); err != nil {
		c.response.clear()
	}
	return
}
...
func (c *Context) Error(err error) {
	c.spark.httpErrorHandler(err, c)
}

func (c *Context) reset(r *http.Request, w http.ResponseWriter, e *Spark) {
	c.request = r
	c.response.reset(w)
	c.query = nil
	c.store = nil
